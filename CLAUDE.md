...
- **학습 문서의 "4. 직접 확인한 것"과 "6. 남은 의문"은 사용자가 채운다.** AI가 대신 쓰면 학습이 되지 않는다.
  AI는 1~3, 5를 초안으로 작성하고 4·6은 **뼈대와 확인용 명령만** 제시한다.
- 사용자가 이해했는지 확인 없이 다음 단계로 넘어가지 않는다. 큰 단계가 끝나면 **무엇을 했고 무엇이 남았는지** 짚는다.
