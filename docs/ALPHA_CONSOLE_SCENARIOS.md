...
### S18. 수락 직후의 지연 — **기다리면 저절로 열린다**

이 콘솔에서 가장 오해받기 쉬운 구간이다. 관측하려면 **수락 시점부터 바로** 찔러야 한다.

**조작** 초대받은 계정으로 로그인 → `/memberships` 에서 수락 → **로그아웃하지 말고** 곧바로
`/t/demo/orders` 를 반복 요청

**기대**

| 시점 | `/iam/memberships` | `/api/orders` + `X-Requested-Tenant` |
|---|---|---|
| 수락 직후 | **ACTIVE** | **403** |
| ~4분 | ACTIVE | 403 |
| **약 4분 뒤** | ACTIVE | **200** ← 아무것도 안 했는데 열린다 |

실측(10초 간격, `scenario-user`):
...
**관측 전에 변수를 좁히는 것이 중요하다.** 75초 시점에 Keycloak group `/tenants/demo` 에 이미
sub 가 들어가 있었다 — 즉 **outbox(첫 번째 겹)는 끝났고 남은 변수는 토큰뿐**이었다. 이걸
확인하지 않으면 "outbox 가 느린 건지 토큰 때문인지" 를 구분할 수 없다.

**증거** 게이트웨이는 BFF 라 refresh token 을 세션에 갖고 있고, `TenantGateFilter` 는 토큰을
`authorizedClientManager.authorize()` 로 읽는다 — **만료를 감지하면 자동 갱신**하는 경로다.
Keycloak 은 토큰 발급 때마다 protocol mapper 를 다시 실행하므로 새 토큰에 갱신된 `groups` 가
실린다. 225~235초는 access token 수명 5분과 정합한다(토큰이 수락보다 먼저 발급됐으므로 잔여
수명은 5분보다 짧다).

> ⚠️ **"저절로 열린다" 를 "즉시 반영된다" 로 읽으면 안 된다.** 회수(revoke) 방향에서는 같은
...
### S20. 회수 지연 — **해제해도 한동안 통과한다**

이 콘솔에서 **유일하게 보안 노출을 직접 보여주는** 시나리오다. 세션이 **두 개** 필요하다.

**준비** 일반 창에 소속 계정(`scenario-user`), 시크릿 창에 관리자(`nativeadmin`)

**조작** 일반 창에서 `/api/orders` 를 반복 요청하는 상태로 두고, 시크릿 창에서 `/admin` →
그 사용자 **해제**
...
09:44:37  Keycloak group /tenants/demo 에서도 제거 확인
09:45:07  멤버십=(없음)   접근=200      ← 마지막 200
09:45:11  멤버십=(없음)   접근=403      ← 차단
```

**위험 구간 ≈ 70초.** 그 사이 `/iam/memberships` 는 비어 있었고 Keycloak group 에서도 빠져
있었다. **DB 도 IdP 도 "멤버가 아니다" 라고 말하는데 요청은 통과했다.**

> ⚠️ **70초라는 숫자를 믿으면 안 된다.** 이 값은 해제 시점에 토큰이 얼마나 남아 있었는지에
> 달렸고, 다시 하면 다른 값이 나온다 — 이론상 **0초~5분**. S18(부여)에서는 235초였다.
> **짧게 나온 것은 안전해서가 아니라 운이 좋아서다.** 설계상 상한인 **5분**으로 이야기해야 한다.

**S18 과 나란히 봐야 의미가 완성된다** — 같은 메커니즘인데 의미가 정반대다:
...
S18 만 보고 "알아서 되니 신경 쓸 것 없다" 로 정리하면 이 방향을 놓친다.
상세는 [`docs/learning/33`](../learning/33-claim-propagation-delay.md) §4.8.
...
| 계정 | 역할 | 비밀번호 | 멤버십 상태 |
|---|---|---|---|
| `scenario-user` | 없음(비관리자) | ✅ | **소속 없음** — S20(회수) 실측에서 해제했다. 다시 쓰려면 재초대·재수락이 필요하다 |
...
| ~~A4~~ | ~~멤버 해제 후 지연(회수)~~ | — | ✅ **완료** → S20. 위험 구간 **약 70초** 관측 |
...
> **A3 는 닫혔고, 그 결과가 A4 의 전제를 바꾼다.** 세션이 살아 있으면 claim 은 **재로그인 없이
> refresh 만으로** 갱신된다(S18, 실측 약 4분). 그래서 "재로그인 강제 수단" 이라는 미해결 항목은
> **처음부터 필요 없는 것**이었다.
>
> 다만 A4 는 여전히 값어치 있다 — **같은 자동 갱신이 회수 방향에서는 위험**이기 때문이다.
> 해제해도 최대 5분간 통과하고, 그 사이엔 아무 신호도 없다.
