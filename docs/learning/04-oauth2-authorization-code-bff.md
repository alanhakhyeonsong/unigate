...
- [ ] refresh token 도 만료되면(Keycloak SSO Idle/Max) 그때 사용자에게는 무엇이 보이는가?
      세션(30분)은 살아 있는데 갱신이 실패하는 구간이 존재하는가?
- [ ] 동시 요청 여러 개가 **동시에** 만료된 토큰을 만나면 refresh 가 몇 번 일어나는가?
      (Keycloak 의 refresh token rotation 과 겹치면 race 가 되는가 — `KEYCLOAK_REALM_SETUP.md` §4.1 이
      Revoke Refresh Token 을 OFF 로 둔 이유와 연결된다)
- [ ]
