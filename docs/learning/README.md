...
- [x] **최대 처리량과 병목 위치** — 이번 수치는 HPA 상한에 막힌 값이다. 노드 여유 확보 후 재측정 필요 → [28](28-k6-loadtest-silent-failures.md) §6
      ...
      모두 풀자 **311 req/s 를 실패 0% 로** 처리했다. 576 req/s 에서 무너진 곳은 게이트웨이가 아니라
      **IAM(replica 1)** 이었고, IAM 을 스케일하자 이번엔 **공유 DB 의 커넥션 슬롯**(max_connections=100)이
      막아 파드가 `CrashLoopBackOff` 로 죽었다. 풀까지 맞추니 **490 req/s · 실패 0%**.
      **게이트웨이 자체의 상한은 여전히 모른다** — 이번엔 포화 전에 부하가 끝났다
